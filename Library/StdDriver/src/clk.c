...
/**
  ...
  * @return     HCLK frequency
  * @details    This function set HCLK frequency. The frequency unit is Hz. The range of u32Hclk is 25 MHz ~ 72 MHz.
  *             The register write-protection function should be disabled before using this function.
  */
uint32_t CLK_SetCoreClock(uint32_t u32Hclk)
{
    uint32_t u32FomSel; /* u32FomSel=0x50(HCLK=72MHz), u32FomSel=0(HCLK=50MHz) */
    ...
    /* Maximum Flash Access Delay for Safe */
    FMC->FATCON = (FMC->FATCON & (~(5 << 4))) | 0x50;
    ...
    if(CLK->CLKSTATUS & CLK_CLKSTATUS_XTL12M_STB_Msk)
    {
        if(u32Hclk == FREQ_72MHZ)
        {
            /* PLL = 144MHz, HCLK = PLL / 2 (This is for using USB(48MHz) with HCLK=72MHz) */
            u32Hclk = CLK_EnablePLL(CLK_PLLCON_PLL_SRC_HXT, u32Hclk * 2);
            u32Div = 2;
        }
        else
    ...
    /* Flash Access Delay */
    FMC->FATCON = (FMC->FATCON & (~(5 << 4))) | u32FomSel;

    CLK_SetHCLK(CLK_CLKSEL0_HCLK_S_PLL, CLK_CLKDIV_HCLK(u32Div));
...
void CLK_SetModuleClock(uint32_t u32ModuleIdx, uint32_t u32ClkSrc, uint32_t u32ClkDiv)
{
    ...
    uint32_t au32DivTbl[2] = {0x0, 0x20};
    ...
    if(MODULE_CLKDIV_Msk(u32ModuleIdx) != MODULE_NoMsk)
    {
        ...
        u32div = (uint32_t)&CLK->CLKDIV + (au32DivTbl[MODULE_CLKDIV(u32ModuleIdx)]);
        /* Apply new divider */
